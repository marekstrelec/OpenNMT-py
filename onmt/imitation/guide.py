...
class Guide(object):
    ...
    def __init__(self, model_path, mode, alpha, fields, test=False):
        assert mode in ['norm_al', 'norm_al2', 'norm_al_binconf', 'norm_al_conf', 'norm_al_conf2', 'sum_al', 'sum_conf']
    ...
    def apply(self, inp, log_probs, idx_from, idx_to):
        ...
        elif self.mode == 'norm_al2':
            return self.apply_norm_alpha2(inp, log_probs, idx_from, idx_to)
        ...
        elif self.mode == 'norm_al_conf2':
            return self.apply_norm_alpha_conf2(inp, log_probs, idx_from, idx_to)
    ...
    def apply_norm_alpha2(self, inp, log_probs, idx_from, idx_to):
        pred_dist, _ = self.predict(inp[idx_from:idx_to])

        conf = self.alpha

        combined_probs = torch.exp((1-conf) * log_probs[idx_from:idx_to]) * torch.exp(conf * pred_dist)
        denom = torch.sum(combined_probs, dim=1)
        combined_norm = torch.log(combined_probs / denom.view(-1, 1))
    ...
    def apply_norm_alpha_conf2(self, inp, log_probs, idx_from, idx_to):
        pred_dist, pred_conf = self.predict(inp[idx_from:idx_to])

        conf = self.alpha * pred_conf
